...
`python3 normalize_srt.py $input-SRT-file.srt $output-SRT-file.srt`

Replace the variables `$input-SRT-file.srt` and `$output-SRT-file.srt` with your files. 

You can then run the main script that will convert the SRT into a TextGrid. 

`python3 SrtToTextgrid.py $output-SRT-file.srt $output-TextGrid-file.TextGrid`

Try experimenting with either the original SRT or the cleaned up SRT:
